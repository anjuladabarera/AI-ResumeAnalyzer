...
const UploadPDFATSScore = () => {
  ...
  // Warn before leaving if a file is selected
  useEffect(() => {
    const handleBeforeUnload = (e) => {
      if (!file) return;
      e.preventDefault();
  ...
  const sendToLLM = async (text) => {
    setLoadingAI(true);
    try {
      const res = await axios.post("/api/cv/ats-score", { text });
      ...
      <div className="max-w-6xl mx-auto animate-fade-in">
        ...
        <div className="card-modern mb-8 animate-fade-in-up" style={{ animationDelay: "0.1s" }}>
          <div className="space-y-4">
            ...
            <button onClick={handleUpload} disabled={loadingUpload || !file} className="btn-primary w-full disabled:opacity-50 disabled:cursor-not-allowed">
              {loadingUpload ? "Uploading..." : "Upload PDF"}
            </button>

            {message && (
              <div className={`p-4 rounded-lg ${message.includes("failed") ? "bg-red-50 border border-red-200" : "bg-green-50 border border-green-200"}`}>
                <p className={`text-sm font-medium ${message.includes("failed") ? "text-red-800" : "text-green-800"}`}>{message}</p>
              </div>
            )}
          </div>
        </div>
        ...
        {extractedText && (
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 animate-fade-in-up">
            {/* Extracted Text */}
            <div className="card-modern">
              <h2 className="text-xl font-bold text-gray-900 mb-2">Extracted Text</h2>
              <div className="max-h-96 overflow-y-auto scrollbar-modern">
            ...
            <div className="card-modern">
              ...
              {loadingAI ? (
                ...
                analysis && (
                  <div className="space-y-6 max-h-96 overflow-y-auto scrollbar-modern">
                    <div>
                      <p className="font-semibold">ATS Score: <span style={{ color: getScoreColor(analysis.ats_score ?? 0) }}>{analysis.ats_score ?? 0}%</span></p>
                      <div className="w-full bg-gray-200 h-4 rounded-full">
                        <div className="h-4 rounded-full" style={{ width: `${analysis.ats_score ?? 0}%`, backgroundColor: getScoreColor(analysis.ats_score ?? 0) }} />
                      </div>
                    </div>
                    ...
                    {analysis.strengths?.length > 0 && (
                      <div>
                        ...
                        <ul className="bg-green-50 border border-green-200 p-2 rounded">
                    ...
                    {analysis.weaknesses?.length > 0 && (
                      <div>
                        ...
                        <ul className="bg-red-50 border border-red-200 p-2 rounded">
                    ...
                    {analysis.missing_keywords?.length > 0 && (
                      <div>
                        ...
                        <div className="flex flex-wrap gap-2">
                          {analysis.missing_keywords.map((k, i) => <span key={i} className="px-2 py-1 text-sm bg-yellow-100 rounded">{k}</span>)}
                        </div>
                      </div>
                    )}
